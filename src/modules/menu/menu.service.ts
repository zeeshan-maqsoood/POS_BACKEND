...
//     // If branch is set to 'global', set isGlobal to true and clear branchId
//     if (itemData.branchId === 'global') {
//       itemData.isGlobal = true;
...
//         }
//       }),
//       // Handle restaurant relation if restaurantId exists
//       ...(itemData.restaurantId && {
//         restaurant: {
//           connect: { id: itemData.restaurantId }
//         }
//       })
//     };
...
//     if ((user?.role === 'MANAGER' && (user?.branch || queryParams?.branchId || queryParams?.branchName)) || queryParams?.isManagerRequest) {
//       let branchId = queryParams?.branchId;
//       let branchName = queryParams?.branchName;
//       const restaurantId = queryParams?.restaurantId;
      
//       // If branchId or branchName is provided in query params, use that
//       if (!branchId && !branchName) {
//         // Otherwise, use the user's branch
//         const branchString = user?.branch && (typeof user.branch === 'string' 
//           ? user.branch 
//           : user.branch?.name || user.branch?.id || '');
          
//         if (branchString) {
//           // Handle both old branch format (branch1, branch2, etc.) and new format (Uptown Branch, etc.)
//           branchName = branchString.startsWith('branch')
//             ? branchString.replace('branch1', 'Main Branch')
//               .replace('branch2', 'Downtown Branch')
//               .replace('branch3', 'Uptown Branch')
//               .replace('branch4', 'Westside Branch')
...
//       // Find the branch by ID or name if we have enough info
//       let branch;
//       if (branchId) {
...
//       // If we have a valid branch or restaurant ID, build the query
//       if (branch || restaurantId) {
//         const targetRestaurantId = branch?.restaurantId || restaurantId;
//         const targetBranchId = branch?.id;
        
//         where.OR = [
//           // Modifiers specific to this branch
//           ...(targetBranchId ? [{ branchId: targetBranchId }] : []),
//           // Global modifiers for this restaurant
//           ...(targetRestaurantId ? [
//             { 
//               AND: [
//                 { branchId: null },
...
//             }
//           ] : []),
//           // Truly global modifiers (no branch or restaurant)
//           { branchId: null, restaurantId: null }
//         ];
        
//         console.log('Filtering modifiers for manager:', { 
//           branchId: targetBranchId, 
//           branchName: branch?.name || branchName,
//           restaurantId: targetRestaurantId 
//         });
//       } else {
//         console.log('No valid branch or restaurant found, showing only global modifiers');
//         // If no branch or restaurant is found, show only truly global modifiers
//         where.AND = [
//           { branchId: null },
//           { restaurantId: null }
//         ];
...
//     console.log('Final where clause for modifiers:', JSON.stringify(where, null, 2));
...
export const menuItemService = {
  async create(data: any, user?: any) {
    ...
    delete createData.categoryId;
    delete createData.branchId;
    delete createData.restaurantId; // Remove this as we're using the relation
    ...
    console.log('Final menu item create data:', createData);
  ...
  async update(id: string, data: any, user?: any) {
    ...
    // Handle restaurant - only include if restaurantId is provided
    if (data.restaurantId) {
    ...
    console.log('Final menu item update data:', updateData);
  ...
  async remove(id: string, user?: any) {
    ...
    if (user?.role === 'MANAGER' && user?.branch) {
      ...
      if (managerBranchId && existingItem.branchId !== managerBranchId) {
        throw new Error('You do not have permission to delete this menu item');
      }
    }
    // Admins can delete any item
