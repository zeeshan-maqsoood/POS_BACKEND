...
exports.reportsService = {
    ...
    async getSalesOverview(params) {
        try {
            ...
            const [ordersByStatus, ordersByType, paymentMethods, dailySales, totalStats, 
            // Add paid orders aggregation
            paidOrdersStats] = await Promise.all([
                // Orders by status
    ...
    async getPaymentReports(params) {
        try {
            ...
            const payments = await prisma_1.default.order.groupBy({
                by: ['paymentMethod', 'paymentStatus'],
                where,
                _count: { _all: true },
                _sum: { total: true }
            });
            const paymentSummary = await prisma_1.default.order.groupBy({
                by: ['paymentMethod'],
                where,
                _count: { _all: true },
                _sum: { total: true }
            });
            const paymentStatusSummary = await prisma_1.default.order.groupBy({
                by: ['paymentStatus'],
                where,
                _count: { _all: true },
                _sum: { total: true }
    ...
    async getInventoryStatus(params) {
        try {
            ...
            const metrics = {
                ...
                itemsByCategory: await prisma_1.default.inventoryCategory.findMany({
    ...
    async getInventoryTransactions(params) {
        try {
            ...
            const transactionSummary = await prisma_1.default.inventoryTransaction.groupBy({
                by: ['type'],
                where,
                _count: { _all: true },
                _sum: { quantity: true }
            });
            const monthlyTransactions = await prisma_1.default.inventoryTransaction.groupBy({
                by: ['createdAt'],
                where,
                _count: { _all: true },
                _sum: { quantity: true },
    ...
    async getMenuPerformance(params) {
        try {
            ...
            const filteredPerformance = categoryId
                ? performanceArray.filter((item) => item.menuItem.categoryId === categoryId)
                : performanceArray;
            return {
                success: true,
                ...
                data: {
                    ...
                    totalRevenue: filteredPerformance.reduce((sum, item) => sum + item.totalRevenue, 0),
                    totalQuantity: filteredPerformance.reduce((sum, item) => sum + item.totalQuantity, 0)
                },
                statusCode: 200
            };
        }
    ...
    async getCategoryPerformance(params) {
        try {
            ...
            const performanceArray = Object.values(categoryPerformance).map((item) => ({
                ...item,
            ...
            return {
                ...
                data: {
                    ...
                    totalRevenue: performanceArray.reduce((sum, item) => sum + item.totalRevenue, 0)
                },
                statusCode: 200
            };
        }
    ...
    async getBranchPerformance(params) {
        try {
            ...
            const branchPerformance = await prisma_1.default.order.groupBy({
                by: ['branchName'],
                where,
                _count: { _all: true },
                _sum: {
            ...
            const performanceWithMetrics = await Promise.all(branchPerformance.map(async (branch) => {
                ...
                const [completedOrders, staffCount, inventoryValue] = await Promise.all([
                    ...
                    // Inventory value
                    prisma_1.default.inventoryItem.aggregate({
    ...
    async getBranchComparison(params) {
        try {
            const { startDate, endDate, user } = params;
            const where = { ...buildWhereClause({ startDate, endDate, user }), paymentStatus: 'PAID' };
            const branchData = await prisma_1.default.order.groupBy({
    ...
    async getStaffPerformance(params) {
        try {
            const { startDate, endDate, branchName, user } = params;
            const where = buildWhereClause({ startDate, endDate, branchName, user });
            const staffPerformance = await prisma_1.default.order.groupBy({
                by: ['createdById'],
                where,
                _count: { _all: true },
                _sum: { total: true },
            ...
            const performanceWithDetails = await Promise.all(staffPerformance.map(async (staff) => {
    ...
    async getStaffActivity(params) {
        try {
            ...
            const staffWithOrders = await prisma_1.default.user.findMany({
                where: {
                    ...where,
            ...
            const staffActivity = staffWithOrders.map(staff => ({
                ...
                ordersToday: staff.orders.filter(order => order.createdAt.toDateString() === new Date().toDateString()).length
            }));
            return {
                success: true,
    ...
    async getRevenueReports(params) {
        try {
            const { startDate, endDate, branchName, user } = params;
            const where = buildWhereClause({ startDate, endDate, branchName, user });
            const revenueData = await prisma_1.default.order.groupBy({
                by: ['createdAt'],
                where,
                _sum: {
                    total: true,
            ...
            const summary = await prisma_1.default.order.aggregate({
                where,
                _sum: {
                    total: true,
                    subtotal: true,
            ...
            const revenueByType = await prisma_1.default.order.groupBy({
                by: ['orderType'],
                where,
                _sum: { total: true },
                _count: { _all: true }
    ...
    async getTaxReports(params) {
        try {
            const { startDate, endDate, branchName, user } = params;
            const where = buildWhereClause({ startDate, endDate, branchName, user });
            const taxData = await prisma_1.default.order.groupBy({
                by: ['createdAt'],
                where,
                _sum: { tax: true },
                orderBy: { createdAt: 'asc' }
            });
            const taxSummary = await prisma_1.default.order.aggregate({
                where,
                _sum: { tax: true },
                _count: { _all: true }
            });
            const taxByBranch = await prisma_1.default.order.groupBy({
                by: ['branchName'],
                where,
                _sum: { tax: true },
                _count: { _all: true }
    ...
    async getDashboardOverview(params) {
        try {
            ...
            const [todayStats, weekStats, monthStats, lowStockCount, pendingOrders, staffActivity] = await Promise.all([
                // Today's stats
                prisma_1.default.order.aggregate({
    ...
    async getCustomerBehaviorAnalytics(params) {
        try {
            const { startDate, endDate, branchName, user } = params;
            const where = buildWhereClause({ startDate, endDate, branchName, user });
            const customerBehavior = await calculateCustomerBehavior(where);
    ...
    async getProductPerformanceByTime(params) {
        try {
            const { startDate, endDate, branchName, user } = params;
            const where = buildWhereClause({ startDate, endDate, branchName, user });
            const orders = await prisma_1.default.order.findMany({
    ...
    async getStaffPerformanceByTime(params) {
        try {
            const { startDate, endDate, branchName, user } = params;
            const where = buildWhereClause({ startDate, endDate, branchName, user });
            const staffPerformance = await calculateStaffPerformanceByShift(where, startDate, endDate);
...
function calculatePeakHours(salesByHour) {
    ...
    const busiest = salesByHour.reduce((max, hour) => hour.sales > max.sales ? hour : max, salesByHour[0]);
    const quietest = salesByHour.reduce((min, hour) => hour.sales < min.sales ? hour : min, salesByHour[0]);
    return {
...
async function calculateCustomerBehavior(where) {
    // Calculate average order processing time
    const ordersWithTimes = await prisma_1.default.order.findMany({
...
function calculateProductPerformanceByTime(orders) {
    const timeSlots = {
...
async function calculateStaffPerformanceByShift(where, startDate, endDate) {
    const shifts = [
    ...
    for (const shift of shifts) {
        ...
        const shiftOrders = await prisma_1.default.order.findMany({
            where: shiftWhere,
            include: {
                createdBy: {
                    select: {
        ...
        const shiftPerformance = Array.from(staffMap.values()).map((data) => ({
            ...data,
...
function calculateTimeSlotPerformance(salesByHour, startHour, endHour) {
...
async function calculateVisitPatterns(where) {
    // Get orders grouped by hour to understand visit patterns
    const hourlyOrders = await prisma_1.default.order.groupBy({
        by: ['createdAt'],
        where,
        _count: { _all: true }
    });
...
function calculateTimeSlotProductPerformance(orders) {
    const timeSlots = {
        breakfast: { start: 6, end: 10 },
        lunch: { start: 11, end: 14 },
        dinner: { start: 18, end: 21 }
...
async function calculateShiftEfficiency(where, startDate, endDate) {
    const shifts = [
        { name: 'Morning (6-14)', start: 6, end: 14 },
        { name: 'Evening (14-22)', start: 14, end: 22 },
        { name: 'Late (22-23)', start: 22, end: 23 }
...
function getTopProductsByTimeSlot(productPerformance, timeSlot) {
    const products = productPerformance
        .filter((product) => product.timeSlots[timeSlot])
        .map((product) => ({
...
function generatePeakHourRecommendations(peakHours, salesByHour) {
...
function generateCustomerBehaviorInsights(customerBehavior, visitPatterns) {
    const insights = [];
...
function generateStaffOptimizationRecommendations(staffPerformance, shiftEfficiency) {
    const recommendations = [];
    // Find the most efficient shift
    const mostEfficientShift = shiftEfficiency.reduce((max, shift) => shift.efficiency > max.efficiency ? shift : shift, shiftEfficiency[0]);
    const leastEfficientShift = shiftEfficiency.reduce((min, shift) => shift.efficiency < min.efficiency ? shift : shift, shiftEfficiency[0]);
