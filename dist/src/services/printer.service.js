...
exports.printerService = exports.PrinterService = void 0;
const node_thermal_printer_1 = require("node-thermal-printer");
const printer_config_1 = require("../config/printer.config");
class PrinterService {
    constructor() {
        this.isConnected = false;
        this.printer = new node_thermal_printer_1.printer(printer_config_1.printerConfig);
        this.initializePrinter();
    }
    async initializePrinter() {
        this.isConnected = await (0, printer_config_1.testPrinterConnection)(this.printer);
        if (!this.isConnected) {
    ...
    async printReceipt(order) {
        ...
        try {
            ...
            if (order.tableNumber)
                this.printer.println(`Table: ${order.tableNumber}`);
            if (order.customerName)
                this.printer.println(`Customer: ${order.customerName}`);
            this.printer.drawLine();
            // ----- ITEMS -----
