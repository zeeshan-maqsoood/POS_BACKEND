...
exports.inventoryService = {
     ...
     */
    async updateInventoryStatus(inventoryItemId) {
        try {
    ...
    async deductInventoryForOrder(order) {
        try {
            ...
            const menuItemMap = new Map(menuItemsWithIngredients.map(item => [item.id, item]));
            ...
            for (const orderItem of order.items) {
                ...
                if (orderItem.modifiers && Array.isArray(orderItem.modifiers)) {
                    for (const modifierData of orderItem.modifiers) {
                        const menuItemModifier = menuItem.modifiers.find((m) => m.modifier.id === modifierData.id);
            ...
            await prisma.$transaction(async (tx) => {
                ...
                for (const [inventoryItemId, deductionData] of ingredientDeductions) {
                    ...
                    if (currentInventoryItem.quantity < totalQuantity) {
                        throw new Error(`Insufficient inventory for ${inventoryItem.name} in branch ${order.branchName}. ` +
     ...
     */
    async checkInventoryAvailability(orderData) {
        try {
     ...
     */
    async getBranchInventory(branchName) {
        try {
            if (!branchName) {
     ...
     */
    async getBranchInventoryReport(branchName, startDate, endDate) {
        try {
            if (!branchName) {
                throw new Error('Branch name is required');
            }
     ...
     */
    async getLowStockAlerts(branchName) {
        try {
            const where = {
                OR: [
            ...
            return lowStockItems.map(item => ({
                ...item,
                needsImmediateAttention: item.status === client_1.InventoryStatus.OUT_OF_STOCK,
...
//# sourceMappingURL=inventory-deduction.service.js.map